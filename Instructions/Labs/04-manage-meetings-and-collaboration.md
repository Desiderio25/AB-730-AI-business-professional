...
1. Open Outlook, select the calendar, and then select the **Copilot** icon on the top right to open the Copilot pane.

1. Enter and submit the following prompt, replacing [time] with a specific time at least 5 minutes from now:

    ```prompt
    Create a Teams meeting invite for a 15-minute LaunchPad project check-in today at [time], with only me as the attendee. Include an agenda covering progress updates, open blockers, and next steps.
    ```

1. Review the meeting details Copilot proposes, then select **Create** to create the event on your calendar.
...
1. Open Teams, select the calendar, and find the LaunchPad Project Check-In meeting you scheduled. Select the event and then select **Edit** to open the meeting details.

1. If not already toggled, toggle the **Teams meeting** option, and then select **Save** to update the meeting details.

1. Select the LaunchPad Project Check-In meeting from the calendar and select **Join** > **Join now** to start the meeting.

1. Select the **More** (•••) icon in the meeting toolbar and select **Record and Transcribe** > **Start recording**.

1. In the **Start recording and transcription** dialog, select the appropriate spoken language, choose what to record under **More options**, and then select **Confirm**.
...
1. Select **Leave** to end the meeting.
...
1. From the Teams calendar, select the meeting you just finished.

1. Select **View recap** to open the meeting recap.
...
1. Select the Copilot button in the meeting toolbar to open Copilot pane.
...
1. With the page open, ask Copilot to expand it into a project hub:
...
1. In Microsoft Copilot, select **Notebooks** and select **+ Create new** or **+ New notebook**.

1. Name the notebook `LaunchPad Project Notebook` and select **Next**.

1. Under **References**, search for your LaunchPad Project Check-In meeting and add it if it appears. Also add the `Quarterly Sales Analysis` workbook from an earlier exercise, if you have it saved to OneDrive.
...
1. Optionally, select **Audio overview** to generate a short spoken summary of the notebook's contents.
