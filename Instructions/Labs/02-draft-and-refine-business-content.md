---
lab:
    title: 'Draft and refine business content with Microsoft Copilot'
    description: 'Use Copilot in Word, the Researcher agent, PowerPoint, and Outlook to draft, refine, and share a client-ready case study.'
    duration: 20
    level: 100
    islab: true
...
- Access to Word, PowerPoint, and Outlook, either through the desktop apps or on the web through [Microsoft 365](https://m365.cloud.microsoft.com) at `https://m365.cloud.microsoft.com`.
...
1. Select the **Copilot** icon on the bottom right of the Word window to open the Copilot pane.
...
1. Review the draft that Copilot generates.

1. Select a paragraph you'd like to improve, then ask Copilot to refine it. For example:

    ```prompt
    Make this paragraph more concise—aim for two sentences.
    ```

1. Ask Copilot to adjust the tone for your audience:
...
1. Once you're happy with the draft, select **Done**, then save the document as `Northwind Case Study`.
...
1. Open Microsoft Copilot [Microsoft Copilot](https://copilot.cloud.microsoft) at `https://copilot.cloud.microsoft`.

1. In the left navigation pane, select **Researcher** from the list of Pinned agents.
...
1. Enter and submit the following prompt:
    ...
    > **Note**: Before starting, the Researcher agent may ask you to clarify details such as the timeframe, region, or types of statistics to prioritize. Answer these questions to refine the research, or enter `go ahead` to let the agent use its best judgment.
...
1. Ask a follow-up question to narrow the scope:
    ...
    ```prompt
    Focus on trends that reduce stockouts and overstock costs.
    ```

1. Ask the agent to prepare the output for your document:

    ```prompt
    Summarize this into two sentences I can add to a customer case study.
    ```

1. Copy the summary and paste it into the **Results** section of your **Northwind Case Study** Word document, then ask Copilot in Word to blend it in smoothly:

    ```prompt
    Integrate this research summary into the results section so it flows naturally with the rest of the paragraph.
    ```

1. When the summary is integrated and flows naturally with the rest of the paragraph, select **Done**.
...
1. Select the **Copilot** icon in the bottom right corner of the PowerPoint window to open the Copilot pane.

1. Above the message box, select the mode dropdown and choose **Allow editing** so Copilot can apply changes directly to your document, instead of **Chat only** mode, which only responds in chat.

1. Reference your Word document in a prompt by typing a forward slash (`/`) followed by the file name:
...
1. Refine the content with a follow-up prompt:
    ...
    ```prompt
    Make the bullet points shorter and more action-oriented.
    ```

1. Select a slide, then ask Copilot to generate a supporting image:

    ```prompt
    Create an image representing a retailer using AI-powered analytics to manage inventory.
    ```

1. Review the image Copilot adds to the selected slide. If needed, refine it or replace it.
...
1. Select the **+** (**Add and manage sources**) button, locate and select the `Northwind Case Study` Word document and the `Northwind Case Study - Overview` presentation, and then confirm that both files appear above the prompt box.

1. Enter the following prompt:

    ```prompt
    Draft an email to my manager letting them know the Northwind Traders case study and presentation are complete and ready for review. Keep the tone professional and concise.
    ```

1. Review the draft and make any edits needed.

1. Highlight the email draft, select the Copilot icon that appears, and select **Get Coaching** from the Copilot options to review suggestions on tone and clarity.

1. Apply any suggestions that improve the message, then address the email to yourself so you can review the final result.

1. Select **Send** twice to send the email.
